...
function genericPrintNoParens(path, options, print) {
    ...
    switch (n.type) {
        ...
        case "Program":
            ...
            parts.push(path.call(function (bodyPath) { return printStatementSequence(bodyPath, options, print, true); }, "body"));
            return lines_1.concat(parts);
...
function printStatementSequence(path, options, print, isToplevel) {
    var filtered = [];
    ...
    var parts = [];
    var lineNum = 1;
    var retainLines = isToplevel && options.retainLines;
    filtered.forEach(function (info, i) {
        ...
        var trueLoc = lines && (options.reuseWhitespace || retainLines) && util.getTrueLoc(stmt, lines);
        if (retainLines) {
            if (trueLoc) {
        ...
        var toPush = [maxSpace(prevTrailingSpace, leadingSpace), printed];
        if (notLast) {
        ...
        parts.push.apply(parts, toPush);
        if (retainLines) {
            lineNum = toPush.reduce(function (prior, part) {
                var linesInPart = ("" + part).split('\n').length - 1;
